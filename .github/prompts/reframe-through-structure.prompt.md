...
to the grammar than to the subject matter. This prompt uses foreign structures
as a **change of lighting**: the same object, different shadows. The goal is not
...
languages or forms. If lat is not connected, say so and offer to work
...
1. **Grasp the subject.** Get the concrete text/sentence/thought. If none is
   given (or nothing is selected in the editor), ask for a passage or a clear
   core statement.
...
3. **Query the catalogue.** `theme` uses a closed vocabulary of cognitive axes;
   ...
   stands in front of the others. Call `list_facets` if unsure
   of the exact strings, then `search_patterns` on that `theme`. Pull details
   with `get_pattern` as needed (focus, feature). Search forms by
   ...
   Query per object and per axis, never from recall of an earlier run, and
   sweep by anchor and by category as well as by theme; see *Coverage* below
   for why one sweep is not enough and what has to be written down.
...
- **Exclude it, and only it.** Pass `exclude_names: ["<that language>"]` to
  `search_patterns` so the structure the text already thinks in is not
...
current set with
`search_patterns { "kind": "form", "category": "Technique" }`
instead of working from a fixed list, and pull `focus` and `feature` with
`get_pattern` before applying one.
...
correlated rows that look like two independent pieces of evidence. Nez Percé
(Nimipuutímt) and Oromo (marked nominative) distort no text — they distort the
count, and the weaker of the two goes invisible behind the stronger. Guard: **no two lenses with the same forced
...
Same axis, two strings, two anchors: keep both. Against that, `Nez Percé
(Nimipuutímt)` and `Oromo (marked nominative)` both read `whether the
participant is the one acting` at `subject` — one string, one anchor, one
finding.

To see every partner of a lens before combining, hand both fields back to the
catalogue:
`search_patterns { "attachment": "subject", "forced_choice": "whether the participant is the one acting" }`.
...
`whether the participant patterns with the object`, `Dyirbal (syntactic
ergativity)` asks `which participant may carry through the chain` — two
strings at the same anchor, and both turn on one and the same merge of actor
and undergoer, the first inside a clause and the second across a chain. On a
single sentence only Basque returns anything; on a chain both fire. Cases like
that are decided on the finished rows, not in advance.
...
| `Basque (Euskara)` | patterns with the object or not | the subject case | ablation | destructive | subject indistinguishable from an undergoer |
...
**A contested lens stays out of the combinations.** A pattern whose `status` is
`contested` has been checked against its source and is defensible; what is
disputed is the reading the lens rests on. Used alone that is harmless — the
row stands by itself, carries its mark, and can be weighed. A combination is
different: its rows are read together as one result, so a disputed lens inside
one contaminates a matrix that looks settled, and the contamination is invisible
in the output. Assemble every combination with
`search_patterns { …, "exclude_contested": true }`; reach for a
contested lens only as a single lens in pass 1.

**A finding that rests on a contested lens is marked wherever it is named.**
Write the status with the name — `` `Hopi` (contested) `` — in the finding
matrix, in the verdict record and in the prose. The entry's `source` field says
what is argued and by whom; the mark is the part that survives being quoted.
...
`Basque (Euskara)` then `Nez Percé (Nimipuutímt)` then `Tagalog`.
Does the subject pattern with the object; is "existing" being equated with
"acting upon"; then focus choice supplies the replacement. A *negative* result
is informative: a subject that falls together with the undergoer means the
sentence has a victim and no causer. The instigator count that stood third —
`Turkish (stacked causatives)`, how many links lie between the speaker and the
hands that acted — is contested and no longer rides in the pincer; ask it as a
single lens in pass 1 where the sentence hides a chain of delegation.

**K3 — source pincer.** The sentence gives inference, report or memory as
observation.
`Tuyuca` then `Ewe (logophoric pronouns)` then `Latin (oratio obliqua)`.
Five sources fanned out, then whether a report speaks of the one quoted or of
another, then the *oratio obliqua* marks the whole passage as report.
...
a point. Two completion questions at two anchors, and a closer.
`Russian` then `Finnish (partitive object)`, plus `Guaraní`.
...
This pincer used to run four diagnostics. Two of them — `Tz'utujil` and `Hopi`
— are contested and have left it, and the catalogue holds no sourced
replacement that is genuinely independent. `Biblical Hebrew` was the obvious
candidate and does not qualify twice over: its forced choice, whether the event
is completed or still running, sits at `verb` beside `Russian` and lands in the
same verdict space on most sentences; and the half of it that *is* distinct —
the prefixed waw reversing a form's value in a chain — rests on an account that
has been challenged, so promoting it would only move the lens from sourced to
contested and out of the pincer again. That is a gap in the catalogue, not a
slot to be filled. Both contested lenses remain available singly in pass 1,
marked.

`Finnish (partitive object)` carries the sharpest
...
`Guaraní` is the reconstruction: the others are diagnostic, and a combination
may not leave role (4) empty. Nominal aspect puts
...
- **parallel:** `Nez Percé (Nimipuutímt)` + `Oromo (marked nominative)` — one
  ...
  else and stays: it and Nez Percé are near-inverses at that anchor — Nez Percé
  refuses the merge of intransitive subject and object that Basque performs, so
  the two return different findings on the same sentence and may be combined.
- **graded:** `Tuyuca` + `Wintu` — one string, `the source of the information`,
  at one anchor. Tuyuca marks all five sources; Wintu marks four and leaves
  plain sight unmarked, so it is the coarser instrument for the same question.
  Either, never both.
- **checked, not a collision:** `Basque (Euskara)` + `Burushaski` — the same
  anchor, and formerly the same string. Burushaski switches marking between a
  controlled act and an involuntary reflex; Basque gates the ergative on clause
  transitivity, not on intent. The shared string was an error in the Basque
  entry, not a shared question. Recorded because the pair comes from comparing
  the two fields rather than from a run, and a reading of `focus` alone would
  still miss it — "System state instead of agency" and "Grading of involuntary
  vs. intentional causation" do not look alike.
...
4. **Input language.** The sentence is English, so exclude English — and only
   ...
   - `search_patterns { "kind": "language", "theme": "Time & aspect", "exclude_names": ["English"] }` → Russian, …
   - `search_patterns { "kind": "language", "theme": "Perspective & reciprocity", "exclude_names": ["English"] }` → Japanese, …
